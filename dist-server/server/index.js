...
import { initializeLogStorage, getLogStorage } from './logStorage.js';
...
const getSocketIOOrigins = () => {
    ...
    if (process.env.NODE_ENV !== 'production') {
        origins.push('http://localhost:5173', 'http://localhost:5174', 'http://127.0.0.1:5173', 'http://localhost:3000', 'http://localhost:3002');
    }
...
io.on('connection', (socket) => {
    ...
    socket.on('initiative:editStart', (initiativeId) => {
        const user = connectedUsers.get(socket.id);
        if (user) {
    ...
    socket.on('initiative:editEnd', (initiativeId) => {
        const user = connectedUsers.get(socket.id);
    ...
    socket.on('initiative:create', (data) => {
...
// ============================================
function logActivity(req, type, description, metadata) {
 ...
 */
async function loadAppConfig(doc) {
    try {
 ...
 */
function matchesUserId(ownerId, currentUserId, currentUserEmail) {
 ...
 */
function canUserDeleteTask(config, userRole, taskOwnerId, initiativeOwnerId, currentUserId, currentUserEmail) {
...
app.post('/api/auth/google', loginLimiter, validate(googleAuthSchema), async (req, res) => {
    try {
...
app.post('/api/auth/register', authenticateToken, validate(registerUserSchema), async (req, res) => {
    try {
...
app.post('/api/users/bulk-import', authenticateToken, validate(bulkImportUsersSchema), async (req, res) => {
    try {
        // Check if user is admin
        if (req.user?.role !== 'Admin') {
...
app.post('/api/sheets/bulk-import', authenticateToken, async (req, res) => {
    try {
        // Check if user is admin
        if (req.user?.role !== 'Admin') {
        ...
        const existingInitiatives = existingRows.map((r) => ({
            id: r.get('id') || '',
        ...
        const generatedInitiatives = [...existingInitiatives];
        ...
        const deduplicatedToAdd = initiativesToAdd.filter((init) => {
            const id = init.id;
...
app.post('/api/sheets/initiatives', authenticateToken, validate(initiativesArraySchema), async (req, res) => {
    try {
        ...
        // Log activity
        const createdCount = deduplicated.filter((init) => !rows.find((r) => r.get('id') === init.id)).length;
...
app.delete('/api/sheets/initiatives/:id', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const doc = await getDoc();
        if (!doc) {
...
app.delete('/api/sheets/tasks/:id', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        let initiativeOwnerId = undefined;
        if (parentId) {
            try {
                ...
                if (initiativesSheet) {
                    ...
                    const initiativeRow = initiativeRows.find((r) => r.get('id') === parentId);
                    if (initiativeRow) {
...
app.post('/api/sheets/initiatives/:id/restore', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const doc = await getDoc();
        if (!doc) {
...
app.post('/api/sheets/tasks/:id/restore', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const doc = await getDoc();
        if (!doc) {
...
app.post('/api/sheets/changelog', authenticateToken, validate(changelogSchema), async (req, res) => {
    try {
        ...
        await sheet.addRows(changes.map((c) => ({
            id: String(c.id || ''),
...
app.post('/api/sheets/tasks', authenticateToken, async (req, res) => {
    try {
        ...
        const rows = await sheet.getRows();
        const existingTaskIds = new Set(rows.map((r) => r.get('id')));
        let syncedCount = 0;
        for (const task of tasks) {
            const existing = rows.find((r) => r.get('id') === task.id);
            if (existing) {
        ...
        if (snapshot.data.length === 0) {
            ...
            if (initiativesSheet) {
                ...
                const initiatives = rows
                    .filter((row) => row.get('id') && !row.get('id').startsWith('_meta_'))
                    .map((row) => {
                    const rowData = {};
...
app.get('/api/sheets/pull', authenticateToken, async (req, res) => {
    try {
        const doc = await getDoc();
        if (!doc) {
            res.status(500).json({ error: 'Failed to connect to Google Sheets' });
...
app.post('/api/slack/webhook', authenticateToken, validate(slackWebhookSchema), async (req, res) => {
    try {
...
app.get('/api/sheets/snapshots', authenticateToken, async (req, res) => {
    try {
        const doc = await getDoc();
        if (!doc) {
            res.status(500).json({ error: 'Failed to connect to Google Sheets' });
...
app.post('/api/sheets/scheduled-snapshot', async (req, res) => {
...
app.get('/api/admin/connected-users', authenticateToken, async (req, res) => {
    try {
...
app.get('/api/admin/login-history', authenticateToken, async (req, res) => {
    try {
        // Only admins can access this endpoint
        if (req.user?.role !== 'Admin') {
            res.status(403).json({ error: 'Admin access required' });
...
app.post('/api/admin/weekly-effort-validation', authenticateToken, async (req, res) => {
    try {
        // Only admins can access this endpoint
        if (req.user?.role !== 'Admin') {
            res.status(403).json({ error: 'Admin access required' });
        ...
        const initiatives = rows.map((r) => {
            const row = {};
            initiativesSheet.headerValues.forEach((header) => {
        ...
        let configSheet = doc.sheetsByTitle['Config'];
        let config = { weeklyEffortValidation: { enabled: true, thresholdPercent: 15 } };
        if (configSheet) {
            await configSheet.loadHeaderRow();
            const configRows = await configSheet.getRows();
            if (configRows.length > 0) {
        ...
        const results = teamLeadIds.map((teamLeadId) => {
            ...
            const totalTeamEffort = teamLeadInitiatives.reduce((sum, i) => sum + (parseFloat(i.actualEffort) || 0), 0);
...
app.get('/api/config/value-lists', authenticateToken, async (req, res) => {
    try {
            ...
            try {
                ...
                if (configData) {
                    ...
                    if (valueLists.priorities) {
                        ...
                        valueLists.priorities.forEach((p) => {
                    ...
                    if (valueLists.initiativeTypes) {
                        ...
                        valueLists.initiativeTypes.forEach((t) => {
                    ...
                    if (valueLists.unplannedTags) {
                        ...
                        valueLists.unplannedTags.forEach((tag) => {
...
app.put('/api/config/value-lists', authenticateToken, async (req, res) => {
    try {
...
app.get('/api/backups/:date', authenticateToken, async (req, res) => {
    try {
        if (req.user?.role !== 'Admin') {
            res.status(403).json({ error: 'Admin access required' });
            return;
        ...
        if (backupService) {
            ...
            const matchingTab = Object.keys(doc.sheetsByTitle).find(title => title.includes(date) && title.startsWith('Snap_'));
            if (!matchingTab) {
            ...
            const count = rows.filter((r) => r.get('id') && !r.get('id').startsWith('_meta_')).length;
            res.json({
                backup: {
                    id: matchingTab,
                    timestamp: date + 'T00:00:00Z',
...
app.post('/api/backups/restore/:date', authenticateToken, async (req, res) => {
    try {
        if (req.user?.role !== 'Admin') {
            res.status(403).json({ error: 'Admin access required' });
            return;
...
app.get('/api/backups/versions/:file', authenticateToken, async (req, res) => {
    try {
        if (req.user?.role !== 'Admin') {
            res.status(403).json({ error: 'Admin access required' });
            return;
...
app.post('/api/backups/restore-version', authenticateToken, async (req, res) => {
    try {
        if (req.user?.role !== 'Admin') {
            res.status(403).json({ error: 'Admin access required' });
            return;
...
app.get('/api/backups/:date/verify', authenticateToken, async (req, res) => {
    try {
        if (req.user?.role !== 'Admin') {
            res.status(403).json({ error: 'Admin access required' });
            return;
...
app.get('/api/backups/:date/download', authenticateToken, async (req, res) => {
    try {
        if (req.user?.role !== 'Admin') {
            res.status(403).json({ error: 'Admin access required' });
            return;
...
app.post('/api/logs/errors', authenticateToken, async (req, res) => {
    try {
...
app.get('/api/logs/search', authenticateToken, async (req, res) => {
    try {
        // Only admins can access logs
        if (req.user?.role !== 'Admin') {
            res.status(403).json({ error: 'Admin access required' });
...
app.get('/api/support/my-tickets', authenticateToken, async (req, res) => {
    try {
        ...
        const userTickets = allTickets.filter(ticket => ticket.createdByEmail === userEmail || ticket.createdBy === userId);
...
app.patch('/api/support/feedback/:id', optionalAuthenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
...
app.post('/api/support/feedback/:id/comments', optionalAuthenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { content } = req.body;
        if (!req.user) {
...
app.get('/api/jira/fields', authenticateToken, async (req, res) => {
    try {
        if (!isJiraConfigured()) {
        ...
        const assetClassFields = fields.filter((f) => f.name.toLowerCase().includes('asset') ||
            f.name.toLowerCase().includes('class') ||
            f.id.startsWith('customfield_'));
        res.json({
            success: true,
            totalFields: fields.length,
            relevantFields: assetClassFields.map((f) => ({
                id: f.id,
                name: f.name,
                custom: f.custom,
                type: f.schema?.type,
