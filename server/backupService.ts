...
export class BackupService {
  ...
  private readonly DATA_PREFIX = 'data/';
  private readonly BACKUPS_PREFIX = 'backups/';
   ...
   */
  private async initStorage(projectId?: string): Promise<void> {
    try {
      ...
      for (const prefix of prefixes) {
        ...
        try {
          ...
          if (exists) {
            ...
            const manifest: BackupManifest = JSON.parse(contents.toString());
   ...
   */
  async getBackupDetails(date: string): Promise<BackupManifest | null> {
    await this.ensureInitialized();
   ...
   */
  async listObjectVersions(filePath: string): Promise<ObjectVersion[]> {
    await this.ensureInitialized();
    ...
    try {
      ...
      for (const file of files) {
        if (file.name !== fullPath) continue;
   ...
   */
  async verifyBackup(date: string): Promise<{
    valid: boolean;
    checked: number;
    failed: string[];
  }> {
   ...
   */
  async getBackupDownloadUrls(date: string): Promise<{
...
export function isBackupServiceEnabled(): boolean {
